...
pub fn precalculated_linear(num: f64, params: &[f64]) -> f64 {
    ...
    if index as usize >= (params.len() - 1) {
...
pub fn conway_game_of_life(num: f64, _params: &[f64]) -> f64 {
...
pub fn smooth_life_sigmoid_smoothed(num: f64, params: &[f64]) -> f64 {
    let birth =
        (sigmoid(num, params[0], params[4], 2.0) + sigmoid(num, params[1], -params[4], 2.0)) - 3.0;
    let survive =
        (sigmoid(num, params[2], params[5], 1.0) + sigmoid(num, params[3], -params[5], 1.0)) - 2.0;
    if birth > survive {
        birth
    } else {
        survive
