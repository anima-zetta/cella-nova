// =============================================================================
// compute_128.wgsl — All compute shaders for the MaceLenia simulation.
// 128x128 grid variant: 32 threads process 128 elements per workgroup.
//
...
// 32 threads cooperatively process 128 elements entirely in workgroup memory.
// ---------------------------------------------------------------------------
...
var<workgroup> ping: array<vec2<f32>, 128>;
var<workgroup> pong: array<vec2<f32>, 128>;
...
fn stockham_butterfly(stage: u32, t: u32, src: ptr<workgroup, array<vec2<f32>, 128>>, dst: ptr<workgroup, array<vec2<f32>, 128>>) {
    let R = 1u << stage;
    let b = t / R;
    let k = t % R;
    let src0 = b * 2u * R + k;
...
// ---------------------------------------------------------------------------
// IFFT row pass: stages in FORWARD order (0,1,2,3,4,5,6)
// ---------------------------------------------------------------------------
@compute @workgroup_size(32)
fn ifft_row_main(
    @builtin(local_invocation_id) local_id: vec3<u32>,
    ...
    // Stages in FORWARD order (0→6) — conjugated twiddles via load_twiddle inverse flag
    stockham_butterfly(0u, t, &ping, &pong);
    stockham_butterfly(0u, t + 32u, &ping, &pong);
    workgroupBarrier();
    stockham_butterfly(1u, t, &pong, &ping);
...
// ---------------------------------------------------------------------------
// IFFT column pass: stages in FORWARD order (0,1,2,3,4,5,6)
// ---------------------------------------------------------------------------
@compute @workgroup_size(32)
fn ifft_col_main(
    @builtin(local_invocation_id) local_id: vec3<u32>,
