// =============================================================================
// compute_1024.wgsl — All compute shaders for the Flow Lenia simulation.
...
// Precomputed 10-bit reversal permutation (0..1023), uploaded once by the host.
...
fn fft_row_main(
    ...
    let rev_t2  = bitrev_lut[t + 256u];
    let rev_t3  = bitrev_lut[t + 512u];
    let rev_t4  = bitrev_lut[t + 768u];
    ping[rev_t]  = fft_data[base + t];
    ping[rev_t2] = fft_data[base + t + 256u];
    ping[rev_t3] = fft_data[base + t + 512u];
    ping[rev_t4] = fft_data[base + t + 768u];
    workgroupBarrier();
    ...
    workgroupBarrier();
    stockham_butterfly(9u, t + 0u, &pong, &ping);
...
fn fft_col_main(
    ...
    ping[rev_t3] = fft_data[(t + 512u) * w + col];
    ping[rev_t4] = fft_data[(t + 768u) * w + col];
    workgroupBarrier();
...
// ---------------------------------------------------------------------------
// normalize_growth: normalize IFFT + apply growth  (bindings 8-13)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// channel_aggregate: per-kernel growth -> per-channel  (bindings 13-18)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// reintegration: semi-Lagrangian advection  (bindings 33-40)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Fused complex multiply + IFFT column pass (unused, kept for compat)
// ---------------------------------------------------------------------------
@compute @workgroup_size(256)
