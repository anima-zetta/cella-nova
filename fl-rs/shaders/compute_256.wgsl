// =============================================================================
// compute_256.wgsl — All compute shaders for the Flow Lenia simulation.
// 256x256 grid variant: 32 threads process 256 elements per workgroup.
//
...
// 32 threads cooperatively process 256 elements entirely in workgroup memory.
// ---------------------------------------------------------------------------
...
var<workgroup> ping: array<vec2<f32>, 256>;
var<workgroup> pong: array<vec2<f32>, 256>;
...
fn stockham_butterfly(stage: u32, t: u32, src: ptr<workgroup, array<vec2<f32>, 256>>, dst: ptr<workgroup, array<vec2<f32>, 256>>) {
    let R = 1u << stage;
    let b = t / R;
    let k = t % R;
    let src0 = b * 2u * R + k;
...
fn fft_row_main(
    ...
    ping[bitrev_lut[t + 64u]]    = fft_data[base + t + 64u];
    ping[bitrev_lut[t + 96u]]    = fft_data[base + t + 96u];
    ping[bitrev_lut[t + 128u]]   = fft_data[base + t + 128u];
    ping[bitrev_lut[t + 160u]]   = fft_data[base + t + 160u];
    ping[bitrev_lut[t + 192u]]   = fft_data[base + t + 192u];
    ping[bitrev_lut[t + 224u]]   = fft_data[base + t + 224u];
    workgroupBarrier();
    ...
    fft_data[base + t + 128u]   = ping[t + 128u];
    fft_data[base + t + 160u]   = ping[t + 160u];
    fft_data[base + t + 192u]   = ping[t + 192u];
    fft_data[base + t + 224u]   = ping[t + 224u];
}
...
fn fft_col_main(
    ...
    ping[bitrev_lut[t + 64u]]    = fft_data[(t + 64u) * w + col];
    ping[bitrev_lut[t + 96u]]    = fft_data[(t + 96u) * w + col];
    ping[bitrev_lut[t + 128u]]   = fft_data[(t + 128u) * w + col];
    ping[bitrev_lut[t + 160u]]   = fft_data[(t + 160u) * w + col];
    ping[bitrev_lut[t + 192u]]   = fft_data[(t + 192u) * w + col];
    ping[bitrev_lut[t + 224u]]   = fft_data[(t + 224u) * w + col];
    workgroupBarrier();
    ...
    fft_data[(t + 128u) * w + col]   = ping[t + 128u];
    fft_data[(t + 160u) * w + col]   = ping[t + 160u];
    fft_data[(t + 192u) * w + col]   = ping[t + 192u];
    fft_data[(t + 224u) * w + col]   = ping[t + 224u];
}
...
fn fused_cmul_ifft_col_main(
    ...
    fft_data[(t + 64u) * w + col]    = complex_mul(fft_data[(t + 64u) * w + col], cm_kernel[(t + 64u) * w + col]);
    fft_data[(t + 96u) * w + col]    = complex_mul(fft_data[(t + 96u) * w + col], cm_kernel[(t + 96u) * w + col]);
    fft_data[(t + 128u) * w + col]   = complex_mul(fft_data[(t + 128u) * w + col], cm_kernel[(t + 128u) * w + col]);
    fft_data[(t + 160u) * w + col]   = complex_mul(fft_data[(t + 160u) * w + col], cm_kernel[(t + 160u) * w + col]);
    fft_data[(t + 192u) * w + col]   = complex_mul(fft_data[(t + 192u) * w + col], cm_kernel[(t + 192u) * w + col]);
    fft_data[(t + 224u) * w + col]   = complex_mul(fft_data[(t + 224u) * w + col], cm_kernel[(t + 224u) * w + col]);
    workgroupBarrier();
